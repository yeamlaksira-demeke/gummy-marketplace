...
// ESCROW PAYMENT PROTECTION SETTINGS
define('ESCROW_ENABLED', true);
define('ESCROW_HOLD_DURATION_DAYS', 30);
define('ESCROW_AUTO_RELEASE', false);
define('PAYMENT_RELEASE_METHOD', 'manual');
...
function get_simulation_for_order(array $order)
{
    ...
    $trackingNumber = !empty($order['tracking_number']) ? $order['tracking_number'] : ('GUMMY-' . str_pad((string) $numericOrderId, 6, '0', STR_PAD_LEFT));
        // Calculate how many MINUTES have passed since the order was created
    ...
    // Handle completed orders first
    if ($order['status'] === 'DELIVERED' || $order['status'] === 'COMPLETED') {
        return [
            'status' => 'delivered',
            'tracking_number' => $trackingNumber,
            'courier' => 'GUMMY Express Logistics',
            'current_location' => 'Delivered to Destination',
    ...
    if ($minutesPassed < 2) {
        return [
            ...
            'tracking_number' => $trackingNumber,
            'courier' => 'GUMMY Express Logistics',
            'current_location' => 'Seller Warehouse (Processing Shipment)',
            'estimated_delivery' => '2-3 Business Days'
        ];
    } elseif ($minutesPassed >= 2 && $minutesPassed < 4) {
        return [
            'status' => 'arrived_at_hub',
            'tracking_number' => $trackingNumber,
            'courier' => 'GUMMY Express Logistics',
            'current_location' => 'Main Sorting Hub (Johannesburg)',
            'estimated_delivery' => '1-2 Business Days'
        ];
    } elseif ($minutesPassed >= 4 && $minutesPassed < 6) {
        return [
            'status' => 'in_transit',
            'tracking_number' => $trackingNumber,
            'courier' => 'GUMMY Express Logistics',
            'current_location' => 'In Transit to Local Distribution Center',
            'estimated_delivery' => '1 Business Day'
        ];
    } else {
        // More than 6 minutes have passed
        return [
            'status' => 'out_for_delivery',
            'tracking_number' => $trackingNumber,
            'courier' => 'GUMMY Express Logistics',
            'current_location' => 'your order is out for delivery',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_received'])) {
    ...
    if ($order) {
        ...
        if ($isDelivery && ESCROW_ENABLED === true) {
...
// Seed courier orders with a simulated tracking number immediately if empty
...
if (!empty($courierOrdersNeedingTracking)) {
    ...
    foreach ($courierOrdersNeedingTracking as $seedOrder) {
        ...
        $assignStmt->execute([$trackingNumber, 'SHIPPED', $seedOrder['id'], $_SESSION['user_id']]);
    }
}
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .footer { background: #097c87; color: white; text-align: center; padding: 20px; margin-top: 40px; }
    </style>
</head>
<body>
    <nav class="navbar">
        ...
        <div class="nav-links">
            ...
            <a href=" seller_order.php">Sells&Earnings</a>
    ...
    <div class="container">
        ...
        <?php else: ?>
            <?php foreach ($orders as $order): ?>
                <?php 
                    //  Captures and matches Courier Delivery variations
                    $method = strtolower($order['delivery_method'] ?? '');
                    $isDelivery = ($method === 'courier' || $method === 'delivery' || strpos($method, 'courier') !== false || strpos($method, 'deliver') !== false);
                ?>
